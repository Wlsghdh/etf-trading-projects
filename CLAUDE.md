...
- **DB 데이터 삭제 절대 금지**: etf2_db, etf2_db_processed의 데이터를 사용자 명시적 승인 없이 절대 삭제하지 마라. 이상치 정리, 중복 제거 등 어떤 명목이든 `DELETE` 쿼리를 자동 실행하지 마라. 이전에 이상치 정리 스크립트가 정상 데이터(최근 주가)까지 삭제하여 DB가 손상된 사고가 있었음.
...
### 데이터 무결성 규칙 (CRITICAL)
- **더미 데이터 절대 사용 금지**: 포트폴리오, 잔고, 수익률, 보유종목 등 모든 금융 데이터는 반드시 KIS API 실데이터만 사용하라. 하드코딩된 숫자, mock 데이터, 임의의 fallback 값을 절대 넣지 마라.
- **데이터 소스 단일화**: 동일한 데이터(예: 현재가, 보유종목)는 반드시 하나의 소스(KIS API)에서만 가져와라. DB fallback으로 다른 값이 섞이면 안 된다.
- **데이터 연결 필수**: 포트폴리오, 잔고, 대시보드, 멀티AI 등 모든 페이지에서 표시되는 금융 데이터는 같은 KIS API 소스를 사용해야 한다. 하나가 바뀌면 연결된 모든 곳이 함께 바뀌어야 한다.
- **가격 데이터 우선순위**: ① KIS API 실시간가 > ② Yahoo Finance > ③ etf2_db (ML 학습용으로만). 프론트엔드에 표시할 가격은 ①②만 사용하라.
- **모의계좌 가상 자금 주의**: KIS 모의계좌의 `total_evaluation_usd` 등 가상 총액을 실제 자산으로 표시하지 마라. 반드시 `보유종목 평가금 + 가용 현금`으로 직접 계산하라.
